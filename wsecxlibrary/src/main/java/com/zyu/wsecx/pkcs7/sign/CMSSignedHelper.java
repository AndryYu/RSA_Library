...
import com.zyu.wsecx.x509.X509CollectionStoreParameters;
...
//        }
        ...
        try {
            return X509Store.getInstance(
                    "AttributeCertificate/" + type, new X509CollectionStoreParameters(certs));
    ...
    X509Store createCertificateStore(
        ...
        try {
            return X509Store.getInstance(
                    "Certificate/" + type, new X509CollectionStoreParameters(certs));
        } catch (IllegalArgumentException e) {
            throw new CMSException("can't setup the X509Store", e);
        }
    }
    ...
    X509Store createCRLsStore(
        ...
        try {
            return X509Store.getInstance(
                    "CRL/" + type, new X509CollectionStoreParameters(crls));
        } catch (IllegalArgumentException e) {
            throw new CMSException("can't setup the X509Store", e);
        }
    }
    ...
    CertStore createCertStore(
        ...
        if (certSet != null) {
            addCertsFromSet(certsAndcrls, certSet);
        }

        if (crlSet != null) {
            addCRLsFromSet(certsAndcrls, crlSet);
        }
    ...
    private void addCertsFromSet(List certs, ASN1Set certSet)
            throws CMSException {
    ...
    private void addCRLsFromSet(List crls, ASN1Set certSet)
            throws CMSException {
        CertificateFactory cf;
