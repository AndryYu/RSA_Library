...
public class ASN1InputStream extends FilterInputStream implements DERTags {
	...
	static int readTagNumber(InputStream s, int tag) throws IOException {
		...
		if (tagNo == 0x1f) {
			...
			// "c) bits 7 to 1 of the first subsequent octet shall not all be
			// zero."
	...
	static int readLength(InputStream s, int limit) throws IOException {
		...
		if (length > 127) {
			...
			if (length >= limit) // after all we must have read at least 1
									// byte
			{
	...
	static DERObject createPrimitiveDERObject(int tag, byte[] bytes) {
