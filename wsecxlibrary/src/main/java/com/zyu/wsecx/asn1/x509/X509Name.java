...
import com.zyu.wsecx.asn1.DERString;
import com.zyu.wsecx.asn1.DERUniversalString;
...
public class X509Name extends ASN1Encodable {
	 ...
	 */
	public static final DERObjectIdentifier STREET = new DERObjectIdentifier(
			"2.5.4.9");
	 ...
	 */
	public static final DERObjectIdentifier NAME_AT_BIRTH = new DERObjectIdentifier(
			"1.3.36.8.3.14");
	...
	@SuppressLint("UseValueOf")
	private static final Boolean TRUE = new Boolean(true); // for J2ME
															// compatibility
	...
	public X509Name(ASN1Sequence seq) {
		...
		while (e.hasMoreElements()) {
			...
			for (int i = 0; i < set.size(); i++) {
				...
				ordering.addElement(DERObjectIdentifier.getInstance(s
						.getObjectAt(0)));
				...
				added.addElement((i != 0) ? TRUE : FALSE); // to allow earlier
															// JDK compatibility
			}
		}
	}
	 ...
	 * process.
	 * <p>
	 * <b>Note:</b> if the name you are trying to generate should be following
	 * a specific ordering, you should use the constructor with the ordering
	 * specified below.
	 * 
	...
	public X509Name(Vector ordering, Hashtable attributes,
		...
		for (int i = 0; i != this.ordering.size(); i++) {
			DERObjectIdentifier oid = (DERObjectIdentifier) this.ordering
					.elementAt(i);

			if (attributes.get(oid) == null) {
				throw new IllegalArgumentException(
						"No attribute for object id - " + oid.getId()
								+ " - passed to distinguished name");
			}

			this.values.addElement(attributes.get(oid)); // copy the hash
															// table
		}
	}
	...
	public X509Name(Vector oids, Vector values, X509NameEntryConverter converter) {
		...
		if (oids.size() != values.size()) {
			throw new IllegalArgumentException(
					"oids vector must be same length as values.");
		}
	 ...
	 * will be processed automatically. <br>
	...
	private DERObjectIdentifier decodeOID(String name, Hashtable lookUp) {
		...
		DERObjectIdentifier oid = (DERObjectIdentifier) lookUp.get(Strings
				.toLowerCase(name));
		if (oid == null) {
	 ...
	 * 
	 * @param reverse
	 *            true if we should start scanning from the end, false
	 *            otherwise.
	 * @param lookUp
	...
	public DERObject toASN1Object() {
		if (seq == null) {
			...
			for (int i = 0; i != ordering.size(); i++) {
				...
				DERObjectIdentifier oid = (DERObjectIdentifier) ordering
						.elementAt(i);
	...
	/**
	 * @param inOrder
	 *            if true the order of both X509 names must be the same, as well
	 *            as the values associated with each element.
	 */
	public boolean equals(Object obj, boolean inOrder) {
		...
		for (int i = 0; i < orderingSize; i++) {
			DERObjectIdentifier oid = (DERObjectIdentifier) ordering
					.elementAt(i);
			DERObjectIdentifier oOid = (DERObjectIdentifier) other.ordering
					.elementAt(i);
	...
	public boolean equals(Object obj) {
		...
		if (ordering.elementAt(0).equals(other.ordering.elementAt(0))) // guess
																		// forward
		{
			start = 0;
			end = orderingSize;
	...
	private void appendValue(StringBuffer buf, Hashtable oidSymbols,
                             DERObjectIdentifier oid, String value) {
		...
		while (index != end) {
			if ((buf.charAt(index) == ',') || (buf.charAt(index) == '"')
					|| (buf.charAt(index) == '\\')
					|| (buf.charAt(index) == '+') || (buf.charAt(index) == '<')
					|| (buf.charAt(index) == '>') || (buf.charAt(index) == ';')) {
				buf.insert(index, "\\");
				index++;
				end++;
	...
	public String toString(boolean reverse, Hashtable oidSymbols) {
		...
		for (int i = 0; i < ordering.size(); i++) {
			if (((Boolean) added.elementAt(i)).booleanValue()) {
				ava.append('+');
				appendValue(ava, oidSymbols, (DERObjectIdentifier) ordering
						.elementAt(i), (String) values.elementAt(i));
			} else {
