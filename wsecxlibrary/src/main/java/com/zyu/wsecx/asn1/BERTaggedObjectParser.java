...
public class BERTaggedObjectParser implements ASN1TaggedObjectParser {
	...
	protected BERTaggedObjectParser(int baseTag, int tagNumber,
	...
	public DEREncodable getObjectParser(int tag, boolean isExplicit)
		...
		case DERTags.SEQUENCE:
			if (_indefiniteLength) {
				return new BERSequenceParser(new ASN1StreamParser(
						_contentStream));
			} else {
				return new DERSequenceParser(new ASN1StreamParser(
						_contentStream));
			}
		case DERTags.OCTET_STRING:
			// TODO Is the handling of definite length constructed encodings
			// correct?
			if (_indefiniteLength || this.isConstructed()) {
				return new BEROctetStringParser(new ASN1StreamParser(
						_contentStream));
			} else {
				return new DEROctetStringParser(
						(DefiniteLengthInputStream) _contentStream);
			}
		}
	...
	public DERObject getDERObject() {
		if (_indefiniteLength) {
			...
			return v.size() == 1 ? new BERTaggedObject(true, _tagNumber, v
					.get(0)) : new BERTaggedObject(false, _tagNumber,
		...
		if (this.isConstructed()) {
			...
			return v.size() == 1 ? new DERTaggedObject(true, _tagNumber, v
					.get(0)) : new DERTaggedObject(false, _tagNumber,
					DERFactory.createSequence(v));
		}

		try {
			DefiniteLengthInputStream defIn = (DefiniteLengthInputStream) _contentStream;
			return new DERTaggedObject(false, _tagNumber, new DEROctetString(
					defIn.toByteArray()));
		} catch (IOException e) {
			throw new IllegalStateException(e.getMessage());
		}
