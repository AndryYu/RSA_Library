 ...
 */
public class TBSCertificateStructure extends ASN1Encodable implements
		X509ObjectIdentifiers, PKCSObjectIdentifiers {
	ASN1Sequence seq;
	...
	public TBSCertificateStructure(ASN1Sequence seq) {
		...
		signature = AlgorithmIdentifier.getInstance(seq
				.getObjectAt(seqStart + 2));
		...
		subjectPublicKeyInfo = SubjectPublicKeyInfo.getInstance(seq
				.getObjectAt(seqStart + 6));

		for (int extras = seq.size() - (seqStart + 6) - 1; extras > 0; extras--) {
			DERTaggedObject extra = (DERTaggedObject) seq.getObjectAt(seqStart
					+ 6 + extras);
