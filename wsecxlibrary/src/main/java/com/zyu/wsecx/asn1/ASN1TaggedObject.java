...
/**
 * ASN.1 TaggedObject - in ASN.1 nottation this is any object proceeded by a [n]
 * where n is some number - these are assume to follow the construction rules
 * (as with sequences).
 */
public abstract class ASN1TaggedObject extends ASN1Object implements
	...
	static public ASN1TaggedObject getInstance(ASN1TaggedObject obj,
			boolean explicit) {
		if (explicit) {
	...
	public DEREncodable getObjectParser(int tag, boolean isExplicit) {
		...
		throw new RuntimeException("implicit tagging not implemented for tag: "
				+ tag);
	}
