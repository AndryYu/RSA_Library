...
import cn.org.bjca.wsecx.core.crypto.params.RSABlindingParameters;
...
public class PSSSigner implements Signer {
	...
	public byte[] generateSignature(boolean isHash) throws CryptoException,
		...
		byte[] dbMask = maskGeneratorFunction1(h, 0, h.length, block.length
				- hLen - 1);
	...
	public boolean verifySignature(byte[] signature,boolean isHash) {
		...
		byte[] dbMask = maskGeneratorFunction1(block, block.length - hLen - 1,
				hLen, block.length - hLen - 1);
		...
		System.arraycopy(block, block.length - sLen - hLen - 1, mDash,
				mDash.length - sLen, sLen);
	...
	private byte[] maskGeneratorFunction1(byte[] Z, int zOff, int zLen,
		...
		if ((counter * hLen) < length) {
			...
			System.arraycopy(hashBuf, 0, mask, counter * hLen, mask.length
					- (counter * hLen));
		}
